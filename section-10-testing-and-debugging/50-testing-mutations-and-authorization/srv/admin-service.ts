...
export default class AdminService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.before('READ', Events, this.filterDeleted)
    ...
    this.on('DELETE', Events, this.onSoftDelete)
  ...
  private async onSoftDelete(req: cds.Request) {
    const { ID } = req.params[0] as { ID: string }

    await UPDATE(Events, ID).set({
      isDeleted: true,
  ...
  private filterDeleted(req: cds.Request) {
    // req.query is a generic Query union in the types; .where lives on SELECT at runtime
    (req.query as any).where({ isDeleted: false })
  }
